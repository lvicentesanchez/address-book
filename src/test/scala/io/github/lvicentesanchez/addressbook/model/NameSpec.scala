...
class NameSpec extends Spec {
  ...
  def is = s2"""
      ...
      ordering should
        ...
        return > 0 if nameA < nameB (lexicographical order, surname first) $lowerThan
        return = 0 if nameA = nameB (lexicographical order, surname first) $equalTo
        return < 0 if nameA > nameB (lexicographical order, surname first) $greaterThan
  ...
  def greaterThan = {
    ...
    Ordering[Name].compare(nameA, nameB) must be_>(0)
  }
